...
단점 : 
- Hooks를 사용한 생명 주기 관리가 클래스형 컴포넌트의 메서드보다 직관적이지 않을 수 있음.
...
class Exam1 extends Component {
  ...
  /* React에서 상태(state)와 변수 (Variable)는 둘 다 데이터 저장과 관련되어 있지만, 각기 다른 역할과 사용방식을 가진다.
     - 상태(state)는 React 컴포넌트에서 데이터가 저장되는 곳으로, 상태값이 변경될 때 마다 컴포넌트가 다시 렌더링 된다.
     - 변수는 컴포넌트 내에서 상태가 아닌 일반 데이터를 저장하기 위해 사용되므로
